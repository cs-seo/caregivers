...
import { actionNextLinks, actionNextNotice } from "@/lib/action-next";
...
export default async function DashboardPage({
  ...
  return (
    <div>
        ...
        <section className="mt-6 rounded-2xl border border-line bg-card p-5">
          ...
          {isFamily ? (
            <div className="mt-4 rounded-xl bg-sage p-3 text-sm">
              <p>{comingUpNextNotice()}</p>
              <ul className="mt-2 flex flex-wrap gap-x-4 gap-y-1">
                {comingUpNextLinks().map((link) => (
                  <li key={link.href}>
      ...
      />
      {isFamily && needsAction[0] ? (
        <div className="mt-4 rounded-xl bg-sage p-3 text-sm">
          <p>{actionNextNotice()}</p>
          <p className="mt-2 flex flex-wrap gap-x-4 gap-y-1">
            {actionNextLinks({ sitHref: needsAction[0].href }).map((link) => (
              <Link key={link.href} href={link.href} className="font-medium text-teal hover:underline">
                {link.label}
              </Link>
            ))}

...
} from "@/lib/special-needs-sa-next";
import {
  companionCareSaNextLinks,
  companionCareSaNextNotice,
  companionCareSaNextShows,
} from "@/lib/companion-care-sa-next";
import {
  nanniesSaNextLinks,
  nanniesSaNextNotice,
  nanniesSaNextShows,
} from "@/lib/nannies-sa-next";
import {
  agedCareSaNextLinks,
  agedCareSaNextNotice,
  agedCareSaNextShows,
} from "@/lib/aged-care-sa-next";
import {
...
} from "@/lib/housekeeping-vic-next";
import {
  nanniesQldNextLinks,
  nanniesQldNextNotice,
  nanniesQldNextShows,
} from "@/lib/nannies-qld-next";
import {
  babysittersQldNextLinks,
  babysittersQldNextNotice,
  babysittersQldNextShows,
} from "@/lib/babysitters-qld-next";
import {
  agedCareVicNextLinks,
...
export async function DirectoryResults({
  ...
  return (
    <div>
      ...
      {showHousekeepingSaNext ? (
      ...
      ) : showSpecialNeedsSaNext ? (
        <div className="mt-4 rounded-xl bg-sage p-3 text-sm">
          <p>{specialNeedsSaNextNotice()}</p>
          <p className="mt-2 flex flex-wrap gap-x-4 gap-y-1">
            {specialNeedsSaNextLinks().map((link) => (
              <Link key={link.href} href={link.href} className="font-medium text-teal hover:underline">
                {link.label}
              </Link>
            ))}
      ...
      ) : showCompanionCareSaNext ? (
        <div className="mt-4 rounded-xl bg-sage p-3 text-sm">
          <p>{companionCareSaNextNotice()}</p>
          <p className="mt-2 flex flex-wrap gap-x-4 gap-y-1">
            {companionCareSaNextLinks().map((link) => (
              <Link key={link.href} href={link.href} className="font-medium text-teal hover:underline">
                {link.label}
              </Link>
            ))}
        ...
        <div className="mt-4 rounded-xl bg-sage p-3 text-sm">
          <p>{nanniesSaNextNotice()}</p>
          <p className="mt-2 flex flex-wrap gap-x-4 gap-y-1">
            {nanniesSaNextLinks().map((link) => (
              <Link key={link.href} href={link.href} className="font-medium text-teal hover:underline">
                {link.label}
              </Link>
            ))}
        ...
        <div className="mt-4 rounded-xl bg-sage p-3 text-sm">
          <p>{agedCareSaNextNotice()}</p>
          <p className="mt-2 flex flex-wrap gap-x-4 gap-y-1">
            {agedCareSaNextLinks().map((link) => (
              <Link key={link.href} href={link.href} className="font-medium text-teal hover:underline">
                {link.label}
              </Link>
            ))}
      ...
      ) : showHousekeepingVicNext ? (
        <div className="mt-4 rounded-xl bg-sage p-3 text-sm">
          <p>{housekeepingVicNextNotice()}</p>
          <p className="mt-2 flex flex-wrap gap-x-4 gap-y-1">
            {housekeepingVicNextLinks().map((link) => (
              <Link key={link.href} href={link.href} className="font-medium text-teal hover:underline">
                {link.label}
              </Link>
            ))}
        ...
        <div className="mt-4 rounded-xl bg-sage p-3 text-sm">
          <p>{nanniesQldNextNotice()}</p>
          <p className="mt-2 flex flex-wrap gap-x-4 gap-y-1">
            {nanniesQldNextLinks().map((link) => (
              <Link key={link.href} href={link.href} className="font-medium text-teal hover:underline">
                {link.label}
              </Link>
            ))}
      ...
      ) : showBabysittersQldNext ? (
        <div className="mt-4 rounded-xl bg-sage p-3 text-sm">
          <p>{babysittersQldNextNotice()}</p>
          <p className="mt-2 flex flex-wrap gap-x-4 gap-y-1">
            {babysittersQldNextLinks().map((link) => (
              <Link key={link.href} href={link.href} className="font-medium text-teal hover:underline">
                {link.label}
              </Link>
            ))}
      ...
      ) : showBabysittersVicNext ? (
        <div className="mt-4 rounded-xl bg-sage p-3 text-sm">
          <p>{babysittersVicNextNotice()}</p>
          <p className="mt-2 flex flex-wrap gap-x-4 gap-y-1">
            {babysittersVicNextLinks().map((link) => (
              <Link key={link.href} href={link.href} className="font-medium text-teal hover:underline">
                {link.label}
              </Link>
            ))}
      ...
      ) : showHousekeepingQldNext ? (
        <div className="mt-4 rounded-xl bg-sage p-3 text-sm">
          <p>{housekeepingQldNextNotice()}</p>
          <p className="mt-2 flex flex-wrap gap-x-4 gap-y-1">
            {housekeepingQldNextLinks().map((link) => (
              <Link key={link.href} href={link.href} className="font-medium text-teal hover:underline">
                {link.label}
              </Link>
            ))}
      ...
      ) : showCompanionCareQldNext ? (
        <div className="mt-4 rounded-xl bg-sage p-3 text-sm">
          <p>{companionCareQldNextNotice()}</p>
          <p className="mt-2 flex flex-wrap gap-x-4 gap-y-1">
            {companionCareQldNextLinks().map((link) => (
              <Link key={link.href} href={link.href} className="font-medium text-teal hover:underline">
                {link.label}
              </Link>
            ))}
      ...
      ) : showDisabilityQldNext ? (
        <div className="mt-4 rounded-xl bg-sage p-3 text-sm">
          <p>{disabilityQldNextNotice()}</p>
          <p className="mt-2 flex flex-wrap gap-x-4 gap-y-1">
            {disabilityQldNextLinks().map((link) => (
              <Link key={link.href} href={link.href} className="font-medium text-teal hover:underline">
                {link.label}
              </Link>
            ))}
      ...
      ) : showCompanionCareVicNext ? (
        <div className="mt-4 rounded-xl bg-sage p-3 text-sm">
          <p>{companionCareVicNextNotice()}</p>
          <p className="mt-2 flex flex-wrap gap-x-4 gap-y-1">
            {companionCareVicNextLinks().map((link) => (
              <Link key={link.href} href={link.href} className="font-medium text-teal hover:underline">
                {link.label}
              </Link>
            ))}
      ...
      ) : showSpecialNeedsQldNext ? (
        <div className="mt-4 rounded-xl bg-sage p-3 text-sm">
          <p>{specialNeedsQldNextNotice()}</p>
          <p className="mt-2 flex flex-wrap gap-x-4 gap-y-1">
            {specialNeedsQldNextLinks().map((link) => (
              <Link key={link.href} href={link.href} className="font-medium text-teal hover:underline">
                {link.label}
              </Link>
            ))}
        ...
        <div className="mt-4 rounded-xl bg-sage p-3 text-sm">
          <p>{nursingVicNextNotice()}</p>
          <p className="mt-2 flex flex-wrap gap-x-4 gap-y-1">
            {nursingVicNextLinks().map((link) => (
              <Link key={link.href} href={link.href} className="font-medium text-teal hover:underline">
                {link.label}
              </Link>
            ))}
      ...
      ) : showPersonalCareQldNext ? (
        <div className="mt-4 rounded-xl bg-sage p-3 text-sm">
          <p>{personalCareQldNextNotice()}</p>
          <p className="mt-2 flex flex-wrap gap-x-4 gap-y-1">
            {personalCareQldNextLinks().map((link) => (
              <Link key={link.href} href={link.href} className="font-medium text-teal hover:underline">
                {link.label}
              </Link>
            ))}

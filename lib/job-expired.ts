...
export function expiredJobRepostHref(job: Pick<ExpiredJobPlace, "specialty" | "city">) {
  return postJobHref({ specialty: job.specialty.slug, city: job.city.slug });
}
...
export function expiredJobRecoveryLinks(job: ExpiredJobPlace) {
  ...
  return [
    { href: expiredJobRepostHref(job), label: "Post a new care request" },
    {
